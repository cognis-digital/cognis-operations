...
**cognis-operations** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cognis-operations` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cognis-operations ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
cognis-operations ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
cognis-operations ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `cognis-operations` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="cognis-operations") for rec in records]   # records = your JSON output
